...
const DeviceParametersFS = {
	...
	UPG: {
		...
		objectdefinition: {
			...
			common: {
				...
				desc: {
					"en": "Request firmware upgrade over internet (WiFi connection must be set), Empty = upgrade to release firmware, TEST or test = upgrade to SYR test branch firmware, HGTEST or hgtest = upgrade to HansGrohe test branch firmware",
					"de": "Fordern Sie Firmware-Upgrade über das Internet an (WiFi-Verbindung muss eingestellt werden), Leere = Upgrade auf Firmware, TEST oder Test = Upgrade auf SYR Test Filial Firmware, HGTEST oder hgtest = Upgrade auf HansGrohe Test Zweig Firmware",
					"ru": "Запросить обновление прошивки через Интернет (WiFi соединение должно быть установлено), Empty = обновление для выпуска прошивки, TEST или тест = обновление до SYR тестовой прошивки, HGTEST или hgtest = обновление до HansGrohe тестовой прошивки",
					"pt": "Solicitar atualização de firmware através da Internet (a conexão WiFi deve ser definida), Empty = upgrade para liberar firmware, TEST ou teste = upgrade para o firmware do ramo de teste SYR, HGTEST ou hgtest = upgrade para o firmware do ramo de teste HansGrohe",
					"nl": "Verzoek firmaware upgrade over internet (WiFi connectie moet worden vastgesteld, Empty = upgrade om firmaware vrij te laten, TEST of test = upgrade aan SYR test firmaware, HGTESTESTESTESTHTHTHTHT",
					"fr": "Demande de mise à niveau du firmware sur Internet (WiFi connection must be set), Empty = upgrade to release firmware, TEST or test = upgrade to SYR test branch firmware, HGTEST or hgtest = upgrade to HansGrohe test branch firmware",
					"it": "Richiedi l'aggiornamento del firmware su Internet (la connessione WiFi deve essere impostata), Empty = aggiornamento al firmware di rilascio, TEST o test = aggiornamento al firmware del ramo di prova SYR, HGTEST o hgtest = aggiornamento al firmware del ramo di test HansGrohe",
					"es": "Solicitud de actualización de firmware a través de Internet (conexión WiFi debe establecerse), Vacío = actualización para el firmware de lanzamiento, TEST o prueba = actualización a SYR firmware de la rama de prueba, HGTEST o hgtest = actualización a HansGrohe",
					"pl": "Request firmware ulepszanie oprogramowania w internecie (WiFi connection must be set), Empty = aktualizacja oprogramowania do oprogramowania firmware, TEST lub test = aktualizacja firmy SYR, HGTEST lub hgtest = aktualizacja firmy HansGrohe",
					"uk": "Запит оновлення мікропрограми через Інтернет (WiFi підключення необхідно встановити), Empty = оновлення для релізу прошивки, TEST або тест = оновлення до SYR тестової галузевої прошивки, HGTEST або hgtest = оновлення до HansGrohe тестової галузевої прошивки",
					"zh-cn": "要求在互联网上升级(必须建立WiFi联系)、Empty=升级,以释放警觉、技术或试验=升级为SYR试验组、HGTEP或 hgtest = 升级为HansGrohe试验组的升级"
				  },
				type: 'string',
				unit: null,
				role: 'state',
	...
	SFV: {
		...
		objectdefinition: {
			...
			common: {
				name: {
					"en": "New server firmware version",
					"de": "Neue Server-Firmware-Version",
					"ru": "Новая версия прошивки сервера",
					"pt": "Nova versão de firmware do servidor",
					"nl": "Nieuwe server firmaware",
					"fr": "Nouvelle version du firmware serveur",
					"it": "Nuova versione firmware del server",
					"es": "Nueva versión de firmware servidor",
					"pl": "Nowy serwer",
					"uk": "Нова версія мікропрограми сервера",
					"zh-cn": "新服务器软件版本"
				},
				desc: {
					"en": "Firmware check command. GetSFV returns 1 if new firmware on release branch is ready,  0 otherwise.   SetSFV/1 requests check. Checking is also performed every 24hrs.",
					"de": "Firmware-Check-Befehl. GetSFV gibt 1 zurück, wenn eine neue Firmware auf dem Release-Zweig bereit ist, sonst 0. SetSFV auf 1 setzen erzeugt eine unmittelbare Anfrage. Die Abfrage erfolgt auch alle 24 Stunden.",
					"ru": "Команда проверки прошивки. GetSFV возвращается 1, если новая прошивка на ветке релиза готова, 0 в противном случае. Проверка запросов SetSFV/1. Проверка также выполняется каждые 24 часа.",
					"pt": "Comando de verificação de firmware. GetSFV retorna 1 se o novo firmware no ramo de liberação estiver pronto, 0 caso contrário. Verificação de pedidos SetSFV/1. A verificação também é realizada a cada 24 horas.",
					"nl": "Firmware controle commando. Zorg dat SFV 1 terugbrengt als er nieuwe firmaware op vrijlating is klaar, 0 anders. SetSFV/1 verzoekt controle. Controleren wordt ook elke 24hrs uitgevoerd.",
					"fr": "Contrôle de firmware. GetSFV retourne 1 si le nouveau firmware sur la branche de sortie est prêt, 0 sinon. SetSFV/1 demande de vérifier. La vérification est également effectuée tous les 24 heures.",
					"it": "Controllo firmware. GetSFV restituisce 1 se il nuovo firmware sul ramo di rilascio è pronto, 0 altrimenti. Controllo delle richieste SetSFV/1. Il controllo viene eseguito anche ogni 24 ore.",
					"es": "Control de firmware. GetSFV vuelve 1 si el nuevo firmware en la rama de liberación está listo, 0 de lo contrario. Comprobación de solicitudes SetSFV/1. El chequeo también se realiza cada 24 horas.",
					"pl": "Firmware check command (ang.). GetSFV zwraca 1 jeśli nowy oprogramowanie na stronie wydawnictwa jest gotowe, 0 inaczej. Aplikacja SSFV/1. Wykonane są także co 24h.",
					"uk": "Команда перевірки прошивок. GetSFV повертає 1, якщо нова прошивка на випускному відділенні готова, 0 іншим чином. Перевірка запитів SetSFV/1. Перевірка також виконується кожні 24 години.",
					"zh-cn": "汽车检查指挥。 GetSFV返回1,如果新公司对释放事务进行警觉,则可另作0.。 页:1 每个24个营也进行了检查。."
				},
				type: 'number',
				unit: null,
				role: 'state',
...
module.exports = {channelsRootNameFS, AdapterChannelsFS, DeviceParametersFS};